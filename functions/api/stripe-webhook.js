...
<body>
  ...
  <p><strong>Timestamp:</strong> ${new Date().toISOString()}</p>
    ...
    // Helper to decrement event inventory for tickets/packages
    async function decrementEventInventory(firebaseProjectId, firestoreApiKey, eventId, cartItems) {
      if (!firebaseProjectId || !firestoreApiKey || !eventId || !cartItems || !cartItems.length) return;
      const eventUrl = `https://firestore.googleapis.com/v1/projects/${firebaseProjectId}/databases/(default)/documents/events/${eventId}?key=${firestoreApiKey}`;
