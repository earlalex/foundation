  ...
  connectedCallback() {
    ...
    this.innerHTML = `
      <div class="pricing-table" style="background: var(--theme-color-surface, #ffffff); border: 1px solid var(--theme-color-border, #bee3f8); border-radius: var(--theme-layout-border-radius, 12px); padding: 2.5rem; text-align: center; margin: 2rem auto; max-width: 450px; font-family: var(--theme-font-font-family, system-ui, sans-serif); box-shadow: var(--theme-layout-box-shadow, 0 4px 6px rgba(0,0,0,0.05));">
        <h2 style="color: var(--theme-color-text-primary, #1a202c); margin-top: 0; font-size: 1.75rem; font-weight: var(--theme-font-heading-weight, 800);">${title}</h2>
        <div style="margin: 1.5rem 0;">
          <span style="font-size: 3rem; font-weight: 800; color: var(--theme-color-primary, #2b6cb0);">${price}</span>
          <span style="color: var(--theme-color-text-secondary, #718096); font-size: 1.1rem;">${period}</span>
        </div>
        <p style="color: var(--theme-color-text-secondary, #4a5568); margin-bottom: 1.5rem; line-height: 1.5; font-size: 0.95rem;">${desc}</p>
