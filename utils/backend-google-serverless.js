// utils/backend-google-serverless.js
...
export async function syncGoogleContactCommunication({ phone, name, type, timestamp, token }) {
  if (!token || !phone) {
    console.error('[Google Contacts Sync Error]: Missing required parameters for contact sync');
    return false;
  }
...
export async function sendCommunicationSummaryEmail({ toEmail, summary, duration, query, response, token }) {
  if (!token || !toEmail) {
    console.error('[Gmail Send Error]: Missing required parameters for email send');
    return false;
  }
