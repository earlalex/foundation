...
export class RadioCoordinator {
  ...
  async getRadioPlaylist() {
    try {
      ...
      const playlist = allContent.filter(item => item.includeInRadioStream === true || item.type === 'podcast');
      ...
      console.warn('[RadioCoordinator]: Could not fetch queued tracks, using default mock playlist.', e.message);
      return [
        {
          id: 'radio-track-1',
